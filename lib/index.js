...
function klayed_out(g, module, skin_data) {
    var nodes = _.map(module.nodes, function(n){
        ...
        else if (template[1]['s:type'] == 'generic')
        {
            ...
            tempclone.pop();
            tempclone.pop();
            tempclone.splice(2, 1);
            var text = n.hasOwnProperty('debug') ? n.debug : n.type;
            var num = text.split(/\n/).length - 1;
            _.forEach(text.split(/\n/), function(str, i)
            {
                var textClone = clone(template[2]);
                textClone[2] = str;
                textClone[1].y = String(Number(textClone[1].y)-(num-i)*10);
                tempclone.push(textClone);
            });
            _.forEach(n.inputPorts,function(p, i)
            {
                var portClone = clone(inPorts[0]);
...
function buildKGraphChild(skin_data, n)
{
        ...
        type == 'generic')
    {
        ...
        if ( type == 'generic') {
            var d = n.hasOwnProperty('debug') ? n.debug : n.type;
            var w = 6*Math.max(d.split(/\n/).map(function(str){return str.length;}));
            ret.labels = []
            _.forEach(d.split(/\n/), function(str, i) {
                ret.labels.push({
                    text : str,
                    x : Number(template[2][1].x),
                    y : Number(template[2][1].y) - 11*i,
                    height : 11,
                    width : (w)
                });
            });
        }
        //if ( n.hasOwnProperty('debug') ) {
        //    ret.labels[0].text = n.debug;
        //}
        return ret;
    }
